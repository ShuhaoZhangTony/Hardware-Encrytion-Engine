...
static DECLARE_WAIT_QUEUE_HEAD(iwq);//first use only one wait queue and check the performance.
/**
...
static int dequeue_manag(struct crypto_priv *localCpg)
{
	...
	do{
		...
		if(RTI!=CTI){
			...
			struct request_handler *rho=list_entry(CTI,struct request_handler,mylist);
			struct request_handler *rhw=list_entry(WTI,struct request_handler,mylist);
			printk(KERN_INFO "RTI is point to %d, CTI is point to %d, WTI is point to %d\n",rh->index,rho->index,rhw->index);
			DFlag=1;
		}		
		...
		//printk(KERN_INFO "D%d is waiting for RTI!=CTI",id);
		wait_event_interruptible(wq,RTI!=CTI||com->exitFlag);
...
{	
	...
	do{
		if (key_size == AES_KEYSIZE_256){ //enable this if-else if HW is ready
			...
			printk(KERN_INFO "Iman finish checking, status is %d ,preStats is %d \n",com->status,com->preStatus);
			cont=0;
...
#endif	
				//struct request_handler *rho=list_entry(RTI,struct request_handler,mylist);
				//struct request_handler *rhk=list_entry(CTI,struct request_handler,mylist);
				//printk(KERN_INFO "count is %d, CTI is now point to %d,RTI is point to %d ",cont,rhk->index,rho->index);
				wake_up_all(&wq);//wake up D man
				printk(KERN_INFO "cont is %d",cont);
				BUG_ON(cont<0||cont>63);
		...
		printk(KERN_INFO "I man is waitting for cpg[0]->IFlag");
		wait_event_interruptible(iwq,cpg[0]->IFlag||com->exitFlag);
		cpg[0]->IFlag=0;
...
static int queue_manag(struct crypto_priv *localCpg)
{
	...
	do {
		...
		if(com->numOfTask<50){
			...
			struct request_handler *rhr=list_entry(RTI,struct request_handler,mylist);
			struct request_handler *rho=list_entry(CTI,struct request_handler,mylist);
			struct request_handler *rhw=list_entry(WTI,struct request_handler,mylist);
			...
			int ret;
				while(!ret)
				ret=wait_event_timeout(wq,WTI->next!=RTI->prev||com->exitFlag,1000);
#endif
		}
		if(QFlag){
			QFlag=0;
			...
			if(key_size == AES_KEYSIZE_256){// enable when HW is ready	
					...
					com->numOfTask++;
					spin_unlock_irq(&com->lockOfN);
					BUG_ON(com->numOfTask>63);
					cpg[0]->IFlag=1;
					wake_up(&iwq);//wake up I manager
					int ret=com->numOfTask;
					if(ret>=10){
						printk(KERN_INFO "more than 10 tasks already, Qman wait for a while");		
						printk(KERN_INFO "Qman wake up from waiting");
						msleep(1000);
					}
