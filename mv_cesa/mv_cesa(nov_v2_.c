...
{	
	...
	do{
		if (key_size == AES_KEYSIZE_256){ //enable this if-else if HW is ready
			...
			BUG_ON(com->status<0||com->status>31);
			...
			printk(KERN_INFO "current wr_buf address is %p",&wr_buf[(com->status)*TAG_SIZE]);
			printk(KERN_INFO "HW process:wrbuf 0 after do_crypt:");
			DMEM32(&wr_buf[0*TAG_SIZE],4096);

			//think about how to let CTI directly point to correct position
			//if H/W can support this, no need to return status number
			//or let irq to handle this
...
static int queue_manag(struct crypto_priv *localCpg)
{
	...
	if(id==0){
		com->PrePIndex=0x1f;
		com->HWsimulate_status=0x1f;//given same simulate value for software testing purpose.
	...
	do {
		...
		if(QFlag){
			...
			printk(KERN_INFO "Q%d before wait_event prePIndex is %d",id,com->PrePIndex);
				wait_event(wq,rh->index==(com->PrePIndex+1)%MaxTag||com->exitFlag);
