...
int do_crypt_software(void *src, void *dst, int len, int flag);
...
static int dequeue_manag(struct crypto_priv *localCpg)
{
	...
	com->status=0x3f;
	com->preStatus=0x3f;
	struct request_handler *rhr;
	struct request_handler *rho;
	struct request_handler *rhw;
	int i, cont;
	do {
		__set_current_state(TASK_INTERRUPTIBLE);
		if (RTI == CTI && key_size == AES_KEYSIZE_256) {
			/*read statuts*/
			cont = 0;
			do {
...
#if 0
				// printk(KERN_INFO "DM : status is %d, prestatus is %d, numOfTask is %d \n", com->status, com->preStatus, com->numOfTask);
				rhr = list_entry(RTI,struct request_handler,mylist);
				rho = list_entry(CTI,struct request_handler,mylist);
				rhw = list_entry(WTI,struct request_handler,mylist);
				printk(KERN_INFO "DM: RTI (%02d), CTI (%02d), WTI (%02d), stat (%02d), prestat (%02d), #task (%02d)\n", rhr->index, rho->index, rhw->index, com->status, com->preStatus, com->numOfTask);
#endif	
				// msleep(100);//use schedule_timeout_uninterruptible
				ndelay(500);
				// msleep(1);
				cont++;
			//}while(cont<100000000&&com->status==com->preStatus&&!com->exitFlag);
			} while(cont < 10000 && com->status == com->preStatus && !com->exitFlag);
			// BUG_ON(com->status < 0 || com->status > 63 || cont >= 10000);
			if (com->status < 0 || com->status > 63 || cont >= 10000) {
				printk(KERN_INFO "ERROR: status = %d; prestatus = %d; cont = %d; numOfTask = %d\n", com->status, com->preStatus, cont, com->numOfTask);
				rhr = list_entry(RTI,struct request_handler,mylist);
				rho = list_entry(CTI,struct request_handler,mylist);
				rhw = list_entry(WTI,struct request_handler,mylist);
				printk(KERN_INFO "DM: RTI points to %d, CTI points to %d, WTI points to %d\n", rhr->index, rho->index, rhw->index);
				msleep(1);
				continue;
			}
			...
			// rho = list_entry(CTI, struct request_handler,mylist);
			// if (rho->index > rhw->index)
			//	printk(KERN_INFO "DEQUEUE: Possible Overflow!");
			// printk(KERN_INFO "DEQUEUE: RTI is point to %d, CTI is point to %d, WTI is point to %d\n",rhr->index,rho->index,rhw->index);
#ifdef debug_no
				//for testing only			
				struct request_handler *rh=list_entry(CTI->prev,struct request_handler,mylist);
				BUG_ON(!rh->index == com->status+1);
		...
		// consume everything on this loop, no need to call wait events
		...
		// do {
			...
			if (DFlag) {
				...
				req = ablkcipher_request_cast(staticRH[rhr->index].request_collector);
				req_ctx = ablkcipher_request_ctx(req);
				pTemp = &wr_buf[rhr->index*TAG_SIZE];
				printk(KERN_INFO "Calling do_crypt_software: src[%p], dst[%p], len[%d], flag[%d]", &rd_buf[rhr->index*TAG_SIZE], ucTemp, req->nbytes, req_ctx->decrypt);
				printk(KERN_INFO "Physical address of dst: %08x", virt_to_phys(pTemp));
				do_crypt_software(&rd_buf[rhr->index*TAG_SIZE], ucTemp, req->nbytes, req_ctx->decrypt);
				if (memcmp(pTemp, ucTemp, req->nbytes)) {
					printk(KERN_INFO "ERROR: software & hardware output different!!! [%d]", rhr->index);
					printk(KERN_INFO "Input Data:");
					DMEM32(&rd_buf[rhr->index*TAG_SIZE], req->nbytes);
					printk(KERN_INFO "Software Output:");
					DMEM32(ucTemp, req->nbytes);
					printk(KERN_INFO "Hardware Output:");
					DMEM32(pTemp, req->nbytes);
					
				}
			#endif
				DFlag = 0;
				// printk(KERN_INFO "dequeue_com being called by Dman, with index of %d,numOfTask is %d", rhr->index, com->numOfTask);
				dequeue_complete_req(rhr->index);
				spin_lock_irq(&com->lockOfN);
				com->numOfTask--;
		...
		// spin_unlock_irq(&com->lockOfN);
		wake_up_all(&wq);
//		printk(KERN_INFO "D%d is waiting for com->numOfTask",id);
#if 0
		int ret;
		int cont=0;
		while(!ret>0){
			if(cont!=0)
			printk(KERN_INFO "D-man stuck here, cont is %d\n",cont);
			cont++%999;
			ret=wait_event_interruptible_timeout(wq,com->numOfTask>0||com->exitFlag,msecs_to_jiffies(15000));
		}
#endif
#ifdef debug
			rhr=list_entry(RTI,struct request_handler,mylist);
			rho=list_entry(CTI,struct request_handler,mylist);
			rhw=list_entry(WTI,struct request_handler,mylist);
			printk(KERN_INFO "IN dequeue: RTI is point to %d, CTI is point to %d, WTI is point to %d\n",rhr->index,rho->index,rhw->index);
#endif	
		wait_event_interruptible(wq, com->numOfTask || com->exitFlag);
//		printk(KERN_INFO "D%d is waking up as com->numOfTask is%d",id,com->numOfTask);
...
static int queue_manag(struct crypto_priv *localCpg)
{
	...
	do {
		...
		if(com->numOfTask < 56) {
			...
			rhw=list_entry(WTI,struct request_handler,mylist);
			if (rho->index == rhw->index) {
				printk(KERN_INFO "OVERTAKE ERROR!");
				printk(KERN_INFO "ENQUEUE: RTI is point to %d, CTI is point to %d, WTI is point to %d\n",rhr->index,rho->index,rhw->index);
			}
#endif
			...
			while(!ret>0){
				if(cont!=0)
				printk(KERN_INFO "Q-man stuck at 1 here, cont is %d\n",cont);
				cont++%999;
				ret=wait_event_interruptible_timeout(wq,com->numOfTask<10||com->exitFlag,msecs_to_jiffies(15000));
#endif
			wait_event_interruptible(wq, com->numOfTask < 10 || com->exitFlag);
		
#ifdef debug_no
			int ret;
				while(!ret)
				ret=wait_event_timeout(wq,WTI->next!=RTI->prev||com->exitFlag,10000);
#endif
		}
...
//		printk(KERN_INFO "Q%d manager is going to wait",id);
	//	wait_event_interruptible(wq,queue->qlen||com->exitFlag);
//		printk(KERN_INFO "Q%d manager is waking up with queue lenth is %d\n",id,queue->qlen);
			...
			while(!ret>0){
				if(cont!=0)
				printk(KERN_INFO "Q-man stuck at 2 here, cont is %d\n",cont);
				cont++%999;
				ret=wait_event_interruptible_timeout(wq,queue->qlen||com->exitFlag,msecs_to_jiffies(15000));
			}
#endif
			wait_event_interruptible(wq, queue->qlen || com->exitFlag);
	} while (!kthread_should_stop());
...
int do_crypt_software(void *src, void *dst, int len, int flags)
{
	u8 *src_walk = (u8 *)src;
	u8 *dst_walk = (u8 *)dst;
	do {
...
do_crypt(void *src, void *dst, int len, u32 flags,u32 index)
{
	...
	if(!flags){
		...
		return do_crypt_software(src, dst, len, flags);
	}
}
...
static int mv_handle_req(struct crypto_async_request *req)
{
	...
	// printk(KERN_INFO "wake up queue, with qlen is %d\n", queue->qlen);
	wake_up_all(&wq);
	return ret;
}
...
static int mv_dec_aes_ecb(struct ablkcipher_request *req)
{
	...
	struct mv_req_ctx *req_ctx = ablkcipher_request_ctx(req);
	struct crypto_queue *queue=&com->queue;
	// printk(KERN_INFO "DECRYPT REQ");
	// printk(KERN_INFO "DECREQ");
	req_ctx->op = COP_AES_ECB;
	req_ctx->decrypt = 1;
...
//	spin_lock_init(&com->lockOfReadI);
	...
	crypto_init_queue(&com->queue, 256);
