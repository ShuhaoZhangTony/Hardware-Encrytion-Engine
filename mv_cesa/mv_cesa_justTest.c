...
//					BUG_ON(localCpg->totalRequestSize > maxData);
		...
		{			
			...
			localCpg->IFlag=1;
			spin_unlock_irq(&localCpg->lockOfIFlag);
			//printk(KERN_INFO "Q%d is going to wake up I with numofTask %d",id,localCpg->numOfTask);
...
//			//printk(KERN_INFO "Q%d nothing to handle sleep",id);	
		...
		/*	if(cpg[0]->terminate&&cpg[1]->terminate){
			...
			//printk(KERN_INFO "Q%d is waking up from doing nothing, queue size is %d, decrypt?%d",id,queue->qlen,localCpg->firstFlag);
			cPointer = localCpg->request_collector;
			goto qacquire;
...
do_crypt(void *src, void *dst, int len, u32 flags)
{
	...
	if (key_size == AES_KEYSIZE_256) {
		...
		// wait_event_interruptible_timeout(wq, (dma_rd_reg(DCSR) & DCSR_WRITE_DONE), 5);
		// if(!(dma_rd_reg(DCSR) & DCSR_WRITE_DONE))
		// {
		...
		printk(KERN_WARNING "do crypt: performance for %d bytes is %ld ns. flag = %08x; counter = %d", len, nSec, flags, counter);
