...
struct request_handler//enhanced list_head
{
...
*/
static struct list_head rhlist;
	...
	do{
		if (key_size == AES_KEYSIZE_256){
			...
			do{
				...
				printk(KERN_INFO "status is%X\n",status);
				//cpu_relax();
				//ndelay(10000);
				msleep(1);
			}while(status==preStatus);
			if(status==0x1F){
				//printk(KERN_INFO "H/W wrong, change status %d to 2 for selftesting",status);
				status=63;				
			}
			int i;
			__set_current_state(TASK_UNINTERRUPTIBLE);
			if(status<preStatus){//over run
			//if preStatus is 62, status is 0 should dequeue 63,0
			//if preStatus is 63, status is 0 should dequeue 0
				for(i=preStatus+1;i<MaxTag;i++){
					getnstimeofday(&start);
					dequeue_complete_req(i);
					getnstimeofday(&end);
					int diff=end.tv_nsec-start.tv_nsec;
					printk(KERN_INFO "In deque_comp time is %d\n", diff);
					spin_lock_irq(&com->lockOfQ);
					RTI=RTI->next;
					if(RTI->next==rhlist.next){
						RTI=RTI->next;
					}
					localCpg->numOfTask--;
				...
				for(i=preStatus+1;i<=status;i++){
					//printk(KERN_INFO "wrbuf after do_crypt:");
					//DMEM32(&wr_buf[i*TAG_SIZE],16);
					//printk(KERN_INFO "start dequeue_complete_req with i =%d",i);
					getnstimeofday(&start);
					dequeue_complete_req(i);
					getnstimeofday(&end);
					int diff=end.tv_nsec-start.tv_nsec;
					printk(KERN_INFO "In deque_comp time is %d\n", diff);
					//printk(KERN_INFO "finish  dequeue_complete_req with i =%d",i);
					spin_lock_irq(&com->lockOfQ);
					RTI=RTI->next;
					if(RTI->next==rhlist.next){
						RTI=RTI->next;
		...
		else{//other keysize do nothing
		}
			printk(KERN_INFO "I manager is going to wait for tasks");
			//int ret;
			//while(!ret)
			//	ret=wait_event_timeout(wq,localCpg->numOfTask||com->exitFlag,MAX_SCHEDULE_TIMEOUT);
			wait_event(wq,localCpg->numOfTask||com->exitFlag);
			printk(KERN_INFO "I manager is waking up");	
		if(com->exitFlag)
			goto Iexit;	
...
static int queue_manag(struct crypto_priv *localCpg)
{
	...
	rhlist.next=&rhlist;
	rhlist.prev=&rhlist;
	static	struct list_head *testPointer; 
	//printk(KERN_INFO "testPointer address is %p",testPointer);
	//printk(KERN_INFO "queue mangaer start");
	int i;
/*	printk(KERN_INFO "1.head.next address is %p, address of head is %p",rhlist.next,rhlist);
	printk(KERN_INFO "2.head.pre address is %p, address of head.next is %p",rhlist.prev,rhlist.next);
	list_add_tail(&staticRH[0].mylist,&rhlist);
	printk(KERN_INFO "3.head.next address after add one entry is %p, address of head.pre is %p",rhlist.next,rhlist.prev);
	list_add_tail(&staticRH[1].mylist,&rhlist);
	testPointer=rhlist.next;
	testPointer=testPointer->next;
	printk(KERN_INFO "4.head.next address after add two entry is %p, address of head.pre is %p, rhlist.next.next is %p",rhlist.next,rhlist.prev,testPointer);
*/
	testPointer=rhlist.next;
	//printk(KERN_INFO "testPointer address is %p",testPointer);
	for(i=0;i<MaxTag;i++){
	//printk(KERN_INFO "%d,%p",staticRH[i]==NULL,&staticRH[i]);
	list_add_tail(&staticRH[i].mylist, &rhlist);
	testPointer=testPointer->next;
	//printk(KERN_INFO "testPointer address is %p",testPointer);
	struct request_handler *rh=list_entry(testPointer,struct request_handler,mylist);
	rh->index=i;
//	printk(KERN_INFO "i is %d, index is %d, index from staticRH is %d\n",i,rh->index,staticRH[i].index);
	}
	printk(KERN_INFO "finish initial tag list");
	WTI=RTI=rhlist.next;//everyone of them point to first one;
#if 0
//testing
	for(i=0;i<200;i++){
	struct request_handler *rh=list_entry(WTI,struct request_handler,mylist);
	printk(KERN_INFO "i is %d, index is %d, index from staticRH is %d\n",i,rh->index,staticRH[i%MaxTag].index);
	WTI=WTI->next;
	if(WTI->next==rhlist.next){
	WTI=WTI->next;
	}
	}
#endif
	do {
			if(WTI->next!=RTI){
				//printk(KERN_INFO "Q is going to prepare one request");
...
//					printk(KERN_INFO "before list_entry; nbytes = %d", req->nbytes);
					...
					//printk(KERN_INFO "rdbuf before dataPrepare:");
					//DMEM32(&rd_buf[rh->index*TAG_SIZE],req->nbytes);
					printk(KERN_INFO "with software en/de, index should always be 0, rh-index is %d",rh->index);
					getnstimeofday(&start);
					DataPrepare(rh->index);
					getnstimeofday(&end);
					//printk(KERN_INFO "rdbuf after dataPrepare:");
					int diff=end.tv_nsec-start.tv_nsec;
					printk(KERN_INFO "In DataPrepare time is %d\n", diff);
					//DMEM32(&rd_buf[rh->index*TAG_SIZE],req->nbytes);
					/*send request to hardware*/
					//printk(KERN_INFO "wrbuf before do_crypt:");
					//DMEM32(&wr_buf[rh->index*TAG_SIZE],req->nbytes);
					//printk(KERN_INFO "now ask do_crypt");
					...
					if (key_size != AES_KEYSIZE_256){// using software simulation for non-256 key
						//printk(KERN_INFO "wrbuf after do_crypt:");
						//DMEM32(&wr_buf[rh->index*TAG_SIZE],req->nbytes);
						//printk(KERN_INFO "dequeu_complete_req being called");
						getnstimeofday(&start);
						dequeue_complete_req(rh->index);
						getnstimeofday(&end);
						int diff=end.tv_nsec-start.tv_nsec;
						printk(KERN_INFO "In dequeue_com time is %d\n", diff);
						//printk(KERN_INFO "dequeu_complete_req being finished");
					}

					else{// only when using hardware en/de WTI is moving
						WTI=WTI->next;
						if(WTI->next==rhlist.next){
							WTI=WTI->next;
						}
						...
						localCpg->numOfTask++;
						spin_unlock_irq(&com->lockOfQ);
						//printk(KERN_INFO "H/W seems have problem, delay for one second and check");
						wake_up_all(&wq);
					}
		...
		//printk(KERN_INFO "Q manager is going to wait");
		wait_event(wq,queue->qlen||com->exitFlag);
		//printk(KERN_INFO "Q manager is waking up");
		__set_current_state(TASK_UNINTERRUPTIBLE);
	} while (!kthread_should_stop());
...
do_crypt(void *src, void *dst, int len, u32 flags,u32 index)
{
	...
	if (key_size == AES_KEYSIZE_256) {
//	printk(KERN_WARNING "do crypt: start hardware encrypt/decrypt; flags = %08x; len = %d, queue size is %d ", flags, len,queue->qlen);
	//dma_wr_reg(RDMATLPS, len|0x00000000); 
	//u32 test;
	//test=len|0x00000000|index<<15;
	if(!flags){
		dma_wr_reg(RDMATLPS, len | 0x00000000| index << 15); // caveat: make sure flags contain direction & start bits
	}else{
		dma_wr_reg(RDMATLPS, len | 0x80000000| index << 15); // caveat: make sure flags contain direction & start bits
		}
		//printk(KERN_INFO "do_crypt finish");	
		return 0;
	}
...
#endif
		printk(KERN_INFO "start software encrypt/decrypt");
		u8 *src_walk = (u8 *)src;
		u8 *dst_walk = (u8 *)dst;
		do {
			if (!flags)
	...
	p->complete = mv_crypto_algo_completion;//static void mv_crypto_algo_completion(int)
		...
		while (len) 
		{
			if (!p->sg_src_left) 
			{
				...
				//printk(KERN_INFO "copy_len is %d, if it's 512, then no need to loop\n",copy_len);
				memcpy(dbuf, sbuf, copy_len);
				//printk(KERN_INFO "finish memcpy from sbuf to dbuf");
...
static inline void dequeue_complete_req(u32 index)	
{
	...
	char *sbuf;
		...
		local_bh_enable();				
		p->complete(index);	
		//printk(KERN_INFO "dequeue_complete being finished");
}
