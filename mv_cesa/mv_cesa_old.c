	...
	do{
		if (key_size == AES_KEYSIZE_256){
			...
			do{
				...
				//printk(KERN_INFO "Interrupt manager:com->status is%ld\n",com->status);
				msleep(100);//use schedule_timeout_uninterruptible
			}while(com->numOfTask&&com->status==com->preStatus&&!kthread_should_stop());
			printk(KERN_INFO "Iman finish checking, status is %d",com->status);
			if(com->preStatus==0x1F){
				//printk(KERN_INFO "starting from 63\n");
				com->preStatus=63;
			}
			...
			if(com->status==1) {
				printk(KERN_INFO "================================com->status is %X",com->status);
				return 0;
			}
		}
		int ret;
		while(!ret)
		ret=wait_event_timeout(wq,com->numOfTask||com->exitFlag,1000);
...
static int queue_manag(struct crypto_priv *localCpg)
{
	...
	int PrePIndex=-1;
	int PreDIndex=-1;
	if(id==0){//ONLY Q0 will prepare the linked list
		/*Prepare head*/
		ListHead=&staticRH[0];
		...
		com->status = dma_rd_reg(DCSR);
		printk(KERN_INFO "Before Qman start read status is%x\n",com->status);
		BUG_ON(com->status!=0x1f);
	do {
		...
		if(WTI->next!=RTI){
			...
			wait_event(wq,WTI->next!=RTI||com->exitFlag);
		}
		if(QFlag){
			QFlag=0;
			...
			if (key_size == AES_KEYSIZE_256){
			printk(KERN_INFO "before wait_event prePIndex is %d",PrePIndex);
			int ret;
			while(!ret)
				ret=wait_event_timeout(wq,rh->index==PrePIndex+1,1000);
			PrePIndex++;
			printk(KERN_INFO "after wait_event prePIndex is %d",PrePIndex);
			wake_up_all(&wq);
			}
		...
		int ret;
		while(!ret)
			ret=wait_event_timeout(wq,queue->qlen||com->exitFlag,1000);
		//printk(KERN_INFO "Q%d manager is waking up",id);
		__set_current_state(TASK_UNINTERRUPTIBLE);
		msleep(1000);
	} while (!kthread_should_stop());
